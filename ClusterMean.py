# -*- coding: utf-8 -*-
"""
Created on Sun May  6 11:12:48 2018
...
'''
'''
 Per POLDACE RODATOCE da BIMBA BALIMBA (  > '_'  )>  <|'_'<| (bimba e puldace coccoli)
...
#                continue
#            
#            for jj in range (minimum):
#              
#                if serieBeh [i-1-jj] != serieBeh[i-1]:
#                     if jj+1 >= 300:
#                         if newminimum > jj+1:
#                             newminimum = jj+1
#                         indexList.append(i)
    ...
    for i in range (len (serieBeh)):    
        if serieBeh[i] == action and serieBeh[i-1] != action:
            ...
            while True:
                if i-1-before == 0:
                    break
                elif serieBeh[i -1 - before] != serieBeh[i-1]:
                    break
                before +=1
            if before < threshold:
                continue
